...
export const banUserTool = {
    ...
    execute: async (input: BanUserInput, context: ToolContext) => {
        ...
        if (dry_run) {
            return {
                success: true,
                message: `DRY RUN: Would ban user ${user_id} until ${expiry}.${reason ? ' Reason: ' + reason : ''}`,
                user_id,
                banned_until: expiry,
            };
        }
        ...
        // Build metadata update if reason provided
        let metadataSql = '';
        if (reason) {
            metadataSql = `, raw_app_meta_data = COALESCE(raw_app_meta_data, '{}'::jsonb) || jsonb_build_object('ban_reason', $3::text)`;
        }

        const sql = `
            UPDATE auth.users
            SET banned_until = $1::timestamptz, updated_at = now()${metadataSql}
            WHERE id = $2
            RETURNING id, banned_until::text;
        `;
        ...
        return {
            success: true,
            message: `User ${user_id} banned until ${expiry}.${reason ? ' Reason recorded.' : ''}`,
            user_id,
